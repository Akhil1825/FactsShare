  ...
  useEffect(
    function () {
      async function getFacts() {
        ...
        let querry = supabase.from("facts").select("*");

        if (currentCategory !== "all")
          querry = querry.eq("category", currentCategory);

        const { data: facts, error } = await querry
          .order("votesInteresting", { ascending: false })
          .limit(1000);

        if (!error) setFacts(facts);
        else alert("There was a prpblem getting data.");
        setIsLoading(false);
      }
      getFacts();
    },
